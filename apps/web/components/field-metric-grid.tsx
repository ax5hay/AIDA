        ...
        >
          ...
          <div className="flex items-baseline justify-between gap-2">
            ...
            {row.pctOfDenominator !== null ? (
              <span className="text-xs tabular-nums text-cyan-400/90">
                {row.pctOfDenominator.toFixed(1)}%
              </span>
            ) : (
          ...
          {row.denominator !== null ? (
            <span className="text-[10px] text-zinc-600">vs denom. {row.denominator}</span>
          ) : null}
        </motion.div>
      ))}
    </div>
